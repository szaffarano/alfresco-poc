package afip.tecno.alfresco.behavior;
...
import afip.tecno.alfresco.model.TecnoRatingsModel;
...
public class Rating implements NodeServicePolicies.OnDeleteNodePolicy, NodeServicePolicies.OnCreateNodePolicy {
	...
	public void init() {
		if (logger.isDebugEnabled())
			logger.debug("Initializing rateable behaviors");
		...
		this.policyComponent.bindClassBehaviour(QName.createQName(NamespaceService.ALFRESCO_URI, "onCreateNode"),
				QName.createQName(TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL,
						TecnoRatingsModel.TYPE_SCR_RATING), this.onCreateNode);
		this.policyComponent.bindClassBehaviour(QName.createQName(NamespaceService.ALFRESCO_URI, "onDeleteNode"),
				QName.createQName(TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL,
						TecnoRatingsModel.TYPE_SCR_RATING), this.onDeleteNode);
	}
	...
	public void computeAverage(ChildAssociationRef childAssocRef) {
		...
		if (nodeService.exists(parentRef)
				&& nodeService
						.hasAspect(parentRef, QName.createQName(
								TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL,
								TecnoRatingsModel.ASPECT_SCR_RATEABLE))) {
		...
		if (children.size() == 0) {
			...
			for (ChildAssociationRef child : children) {
				if (!child.getTypeQName().isMatch(
						QName.createQName(TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL,
								TecnoRatingsModel.ASSN_SCR_RATINGS))) {
					continue;
				}
				int rating = 0;
				rating = (Integer) nodeService.getProperty(child.getChildRef(), QName.createQName(
						TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL, TecnoRatingsModel.PROP_RATING));
				count += 1;
				total += rating;
			}
		...
		nodeService.setProperty(parentRef, QName.createQName(TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL,
				TecnoRatingsModel.PROP_AVERAGE_RATING), average);
		nodeService.setProperty(parentRef, QName.createQName(TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL,
				TecnoRatingsModel.PROP_TOTAL_RATING), total);
		nodeService.setProperty(parentRef, QName.createQName(TecnoRatingsModel.NAMESPACE_TECNO_RATINGS_CONTENT_MODEL,
				TecnoRatingsModel.PROP_RATING_COUNT), count);
