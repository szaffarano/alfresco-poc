...
function computeAverage(childAssocRef) {
    ...
    if (parentRef.hasAspect("{http://tecno.afip.gob.ar/model/ratings/1.0}rateable")) {
    ...
    if (children != null && children.length > 0) {
      for (i in children) {
        ...
        var rating = child.properties["{http://tecno.afip.gob.ar/model/ratings/1.0}rating"];
        total += rating;
      }
    ...
    parentRef.properties["{http://tecno.afip.gob.ar/model/ratings/1.0}averageRating"] = average;
    parentRef.properties["{http://tecno.afip.gob.ar/model/ratings/1.0}totalRating"] = total;
    parentRef.properties["{http://tecno.afip.gob.ar/model/ratings/1.0}ratingCount"] = children.length;
    parentRef.save();
...
function getUserRating(curNode, curUser) {
  ...
  if (results == undefined || results.length == 0) {
    ...
    var rating = results[results.length-1].properties["{http://tecno.afip.gob.ar/model/ratings/1.0}rating"];
    if (rating == undefined) {
      return 0;
    } else {
      return rating;
...
function getRating(curNode, curUser) {
  var rating = {};
  rating.average = curNode.properties["{http://tecno.afip.gob.ar/model/ratings/1.0}averageRating"];
  rating.count = curNode.properties["{http://tecno.afip.gob.ar/model/ratings/1.0}ratingCount"];
...
function deleteRatings(curNode) {
    ...
    // check the parent to make sure it has the right aspect
    if (curNode.hasAspect("{http://tecno.afip.gob.ar/model/ratings/1.0}rateable")) {
      // continue, this is what we want
    } else {
